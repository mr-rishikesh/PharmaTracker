...
async  function handleChangeQuantity(e:any) {
      ...
      if(newQty > 100 || newQty < 0 ) return;
      setQuantity(newQty)
      
      let price = selectedMedicine.price;
      let totalCost = newQty*price;
      let discounted = Number(totalCost * ((100 - discount)/100.0));
  ...
  function handleDiscountChange(e:any) {
     let newDisc = (e.target.value);
     if(newDisc > 100 || newDisc < 0 ) return;
      setDiscount(prev => e.target.value);
      let price = selectedMedicine.price;
      let totalCost = quantity*price;


      let discounted = totalCost * ((100 - newDisc)/100.0);

      setTotalPrice(prev => discounted)
  ...
  const handleSelectSuggestion = (med: any, idx: number, unit: string) => {
    if (unit == "expired") {
      setSuggestions((prev) => {
        const newSuggestion = prev.filter(
          (item) => item.batchNo !== med.batchNo
        );

        return newSuggestion;
      });
    } else {
      setInputMedicineName((prev) => med.name);
      setselectedMedicine((prev) => med);
      setSuggestions([]);
    }
  };

  const { addBillItem , billMedicineItems} = UseMedicineStore();
  let length =1;

  function handleAddMedicine() {
    if(selectedMedicine.batchNo == "" || Number(quantity)  < 1) {
      alert("select medicine or quantity")
      return;
    }
       addBillItem( {
       sNo : billMedicineItems.length + 1 ,
       price : totalPrice ,
       quantity : quantity ,
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <input
                ...
                type="text"
                placeholder="Enter Medicine Name or Batch No"
                ...
                <ul className="absolute z-10 bg-white border border-gray-300 rounded-lg mt-1 w-full max-h-60 overflow-y-auto shadow-md">
                  {suggestions.map((med, idx) => {
                    ...
                    return (
                      <li
                        key={idx}
                        onClick={() => handleSelectSuggestion(med, idx, unit)}
                      ...
                      >
                        ...
                        {unit == "expired" ? (
                          <p className="justify-end inline hi">
                            Expired Click To Remove
                          </p>
                        ) : (
                          <p className="justify-end inline hi">
                            Expired in {value} {unit}{" "}
                          </p>
                        )}
                      </li>
                    );
            ...
            <input
              ...
              readOnly
              value={selectedMedicine && selectedMedicine.batchNo}
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
              <div className="flex-1">
                ...
                <input
                  ...
                  className="bg-gray-100 w-full px-3 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                />
              </div>
              <div className="flex-1">
                <label className=" text-sm sm:text-base">Total Quantity</label>
                <input
                  type="number"
                  disabled
            ...
            <button onClick={handleAddMedicine} className="w-full py-2 rounded-lg bg-blue-600 text-white font-semibold hover:bg-blue-700 transition">
              Add
            </button>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="overflow-hidden rounded-lg border border-gray-200">
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {billMedicineItems.length > 0 &&  billMedicineItems.map(item => {
                          return  <BillItem       key={item.sNo}
                sNo={item.sNo}
